...
Git is a version control system or VCS that works by taking a snapshot of your project when you make a repository. When you commit changes to your application Git saves the changes you made into a new update.

            </Essay>
            <Essay question='Q3 what is the diffrence between JQuery and react?'>
            The difference between JQuery and React is that JQuery works to make larger components of code into smaller components. React is built to make webpages quickly and uses components to do so. JQuery is focused on doing more in less lines of code and focuses on making Javascript easier to use on a wb application. It also simplifies the complicated things from javascript. JQuery can maplute most aspects of html, css, and javascrip.

While JQuery is for javascript and animation, React focus on quick and efficient user interface and web applications. React uses components to assemble web applications. These components are responsible for making parts of the UI. React is mostly used to create UI.

            </Essay>
            <Essay question='Q4: What is the difference between a Front-End and Back-End Developer?'>
            Frontend developers work on websites by adding user interaction and styling to websites. They work on the parts that the user will see and interact with and work with html, css, and javascript. Frontend developers work to create user-friendly websites and UI. The average pay of a frontend developer is $78,516 as of 2023. The demand for frontend developers is projected to grow 23% till 2031.
...
Backend developers are responsible for the optimization and backend of the website. They make the website work behind the scenes managing server side data and APIs. Backend developers also do quality assurance testing. Which is testing every part of the website under different interactions and circumstances to make sure that bugs are rare and not website breaking. Backend developer use such programming languages as ruby, PHP, sql, and c#.
            ...
            Some ways that we can link our files and resources is by using a API and Directly searching the computer for the file or resource. APIs work by having the application send a request to the API and the api will ask to fetch the data from the server. APIs need API keys to allow the website or application to get the data. There are different types of APIs for different uses. Representational state transfer API’s or “REST APIs” work by using a url to make the data request and the data is predefined. There are also SOAP APIs, or “or “Simple Object Access Protocol,” is more secure but not as REST APIs.

We can also use packages to link files and resources. Packages are downloadable from the web or the ide and contain related code functions and files to help with adding additional functionality and features to the application.
